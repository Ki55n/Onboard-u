package com.app.lifetidy.utils.prettyDialog
...
import com.example.wifimodule.databinding.DialogPrettyBinding
import com.osquare.prettyDialog.PrettyDialogButton
...
class PrettyDialog(internal var context: Context) : AppCompatDialog(context) {
    ...
    private fun setupViews_Base() {
        ...
        resources?.getDimensionPixelSize(R.dimen._40sdp)?.div(2)?.let { lp.setMargins(0, it, 0, 0) }
        dialogPrettyBinding.llContent?.layoutParams = lp
        resources?.let {
            dialogPrettyBinding.llContent?.setPadding(0, (1.25 * it.getDimensionPixelSize(R.dimen._40sdp) / 2).toInt(), 0, it.getDimensionPixelSize(R.dimen._10sdp))
        }
