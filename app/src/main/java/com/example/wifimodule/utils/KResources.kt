...
fun ObservableField<String>.isPAN() =
    Pattern.compile("[A-Z]{5}[0-9]{4}[A-Z]").matcher(get()).matches()

fun ObservableField<String>.isValidPassword() =
    Pattern.compile("^(?=.*?[A-Za-z])(?=.*?[0-9]).{6,20}").matcher(get()).matches()

fun ObservableField<String>.isValidMobile() =
    Pattern.compile("^[6-9]\\d{9}$").matcher(get()).matches()
...
fun String.isValidPassword() =
    Pattern.compile("^(?=.*?[A-Za-z])(?=.*?[0-9]).{6,20}").matcher(this).matches()
