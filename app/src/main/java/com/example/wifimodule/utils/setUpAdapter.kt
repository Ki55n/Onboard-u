...
fun <ITEM, U : ViewBinding> setUpAdapter(
    ...
    return KSelectionAdapter(items, choiceMode, bindHolder, itemClick, isToggleEnable)
}

open class KSelectionAdapter<ITEM, U : ViewBinding>(
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder<U> {
        ...
        return ViewHolder(
            inflateMethod.invoke(null, LayoutInflater.from(parent.context),parent,false) as U
        )
    }
    ...
    class ViewHolder<out V : ViewBinding>(internal val binding: V?) :
        RecyclerView.ViewHolder(binding?.root!!)

    fun makeFirstItemDefault(makeDefault: Boolean) {
        if (isToggleEnable && items.isNotEmpty()) {
            toggleItemView(0)
        }
    }
    ...
    fun getSingleSelectionToggledViewIndex(): Int {
        for (i in 0 .. items.size){
            if(selectedItemViews.get(i, false)){
                return i
            }
        }
        return -1
    ...
    fun getSelectedItems(): MutableList<ITEM> {
        ...
        if (this.items.isNotEmpty() && getSelectedItemViews().size <= this.items.size) {
    ...
    fun selectAll() {
        items.forEachIndexed { it, item ->
            if (!isItemViewToggled(it)) {
                selectedItemViews.put(it, true)
            }
        }
        notifyDataSetChanged()
    }
    ...
    fun deSelectItem(pos:Int) {
        if(isItemViewToggled(pos)){
            selectedItemViews.delete(pos)
        }
        notifyDataSetChanged()
    }
    ...
    private fun removeSelectedItemView(position: Int) {
        ...
        for (selectedPosition in selectedPositions) {
            selectedItemViews.put(
                if (position > selectedPosition) selectedPosition else selectedPosition - 1,
                true
            )
        }
    }
    ...
    fun itemScrollTouchManage(recyclerView: RecyclerView) {
        recyclerView.addOnItemTouchListener(object : RecyclerView.OnItemTouchListener {
            ...
            override fun onInterceptTouchEvent(view: RecyclerView, event: MotionEvent): Boolean {
                ...
                if (view.canScrollHorizontally(RecyclerView.FOCUS_FORWARD)) {
                    when (action) {
                        MotionEvent.ACTION_MOVE -> recyclerView.parent
                            .requestDisallowInterceptTouchEvent(true)
                    }
                    return false
                } else if (view.canScrollHorizontally(RecyclerView.FOCUS_BACKWARD)) {
                    when (action) {
                        MotionEvent.ACTION_MOVE -> recyclerView.parent
                            .requestDisallowInterceptTouchEvent(true)
                    }
