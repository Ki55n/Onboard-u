...
import com.example.wifimodule.databinding.ActivityGstOtpBinding
import com.example.wifimodule.presentation.login.LoginActivity
import com.example.wifimodule.presentation.signup_complete.SignUpCompleteActivity
import com.example.wifimodule.utils.simpleAlert
import dagger.hilt.android.AndroidEntryPoint

@AndroidEntryPoint
class GstOtpActivity : BaseActivity<ActivityGstOtpBinding, GstOtpActivityVM>() {
    ...
    private fun handleOTP(it: Resource<OTPResponse>?) {
        if (it is Resource.Success) {
            if (it?.data?.status?.code?.equals("200") == true) {
                startActivity(Intent(this@GstOtpActivity, SignUpCompleteActivity::class.java).
