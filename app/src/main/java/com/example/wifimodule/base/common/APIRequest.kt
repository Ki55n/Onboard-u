    ...
    return try {
        ...
        if (response.isSuccessful and (response.code() == 200)) {
            ...
            if (response.code() == 401) {
                SharedPrefs(BaseApp.INSTANCE).clearAll()
                BaseApp.INSTANCE.startActivity(
                    Intent(BaseApp.INSTANCE, LoginActivity::class.java).setFlags(Intent.FLAG_ACTIVITY_NEW_TASK or Intent.FLAG_ACTIVITY_CLEAR_TASK).putExtras(Bundle.EMPTY))
            }
            var errorBody = response.errorBody()?.string()
                ?: throw IllegalStateException("Error body is null")
            try {
                val type = object : TypeToken<LoginResponse>() {}.type
                val err = Gson().fromJson<LoginResponse>(errorBody, type)
                errorBody = err.message.toString() ?: ""
            } catch (e: Exception) {
                e.printStackTrace()
            }
