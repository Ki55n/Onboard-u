...
abstract class BaseFragment<DB : ViewBinding, VM : BaseViewModel> : Fragment() {
    ...
    open val isOptionMenuEnable: Boolean?
        get() = false
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        var coreActivity = requireActivity()
        if (coreActivity is BaseActivity<*, *>) {
            if (coreActivity?.supportActionBar != null) {
                coreActivity?.initToolbarBack(isBackEnable)
//                coreActivity?.viewModel?.title?.value = title
                coreActivity?.initToolbarTitle(titleToolbar)
            }
        }
